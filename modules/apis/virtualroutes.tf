...
resource "null_resource" "deploy_nsm" {
  ...
  provisioner "local-exec" {
    command = "nginx-meshctl deploy --disable-auto-inject --enabled-namespaces=\"bookinfo,microservice-namespace\" --mtls-mode strict --registry-server ${var.nginx_service_mesh_registry_name} --image-tag ${var.nginx_service_mesh_version} --deploy-grafana=false --prometheus-address prometheus-service.monitoring:8080"
